...
<body>
	...
	<%
		...
		int result = bbsDAO.hitPlusPlus(bbsID);
	%>
		...
		<div class="row">
			<table class="table table-striped" style="text-align: cetner; board: 1px solid #dddddd">
				...
				<tbody>
					...
					%>
						<tr>
							<td colspan="2" style="text-align: center;"><%=bbs.getBbsFileName() %></td>
						</tr>
					<%
						} else {
					%>
						<tr>
							<td colspan="2" style="text-align: center;"><a href="fileDownload.jsp?file_name=<%= bbs.getBbsFileName() %>"><%=bbs.getBbsFileName() %></a></td>
						</tr>
					<%
						}
					%>
